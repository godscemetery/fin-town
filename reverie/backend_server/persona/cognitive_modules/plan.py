...
def generate_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
    """
    生成某个被使用对象的状态描述。
    这里对 run_gpt_prompt_act_obj_desc 的返回做统一规范化：
    - 允许它返回 (output, debug_info)
    - 也允许它返回字符串
    - 其余情况一律兜底为 "<obj> is idle"
    """

    # 调用底层的 LLM 包装函数
    res = run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose)

    # 情况 1：原始设计：返回 (output, debug_info)
    if isinstance(res, (list, tuple)):
        # 比如 ('being fixed', [...])
        if len(res) > 0 and isinstance(res[0], str) and res[0].strip():
            return res[0].strip()
...
def _determine_action(persona, maze): 
  ...
  # ===== 新增：如果还没有任何 fine-grained schedule，就塞一条默认行为，防止后面取空表 =====
  if not persona.scratch.f_daily_schedule:
    # 没有细分日程时，让 agent 原地思考市场 60 分钟（只是一条安全兜底，不影响后面扩展）
    persona.scratch.f_daily_schedule = [
      ["stay at the current location and quietly think about the financial market.", 60]
    ]
    curr_index = 0
    curr_index_60 = 0
    persona.scratch.curr_action_index = 0
  # ===== 新增结束 =====
  ...
  # ===== 新增：再保险一下，防止 curr_index 仍然越界 =====
  if curr_index >= len(persona.scratch.f_daily_schedule):
    curr_index = len(persona.scratch.f_daily_schedule) - 1
    persona.scratch.curr_action_index = curr_index
  # ===== 新增结束 =====
