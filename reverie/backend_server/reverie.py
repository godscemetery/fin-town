...
class ReverieServer:
    def __init__(self,
        ...
        # simulation.
        self.medical_chat_path = f"{sim_folder}/chat.md"
        self.medical_chat_seen = set()
        if check_if_file_exists(self.medical_chat_path):
            try:
                with open(self.medical_chat_path) as md_file:
                    for line in md_file:
                        line = line.strip()
                        if line.startswith("<!-- chat-key: "):
                            key = line.replace("<!-- chat-key: ", "")
                            key = key.replace(" -->", "").strip()
                            if key:
                                self.medical_chat_seen.add(key)
            except Exception:
                pass
    ...
    def _make_medical_chat_key(self, step, chat):
        if chat is None:
            return None
        parts = []
        for row in chat:
    ...
    def _extract_medical_chat_records(self, movements):
        records = []
        seen_in_step = set()
        for persona_name, pdata in movements.get("persona", {}).items():
            chat = pdata.get("chat")
            if not chat:
                continue
            signature = tuple(
                (row[0], row[1]) for row in chat if row and len(row) >= 2
            )
            if not signature or signature in seen_in_step:
                continue
            participants = []
            for row in chat:
                if not row or len(row) < 2:
    ...
    def _append_medical_chat_records(self, records):
        if not records:
            return
        lines = []
        if not check_if_file_exists(self.medical_chat_path):
            lines.append("# Medical Chat Records")
            lines.append("")
        for record in records:
            key = self._make_medical_chat_key(record.get("step"),
                                              record.get("chat"))
            if not key or key in self.medical_chat_seen:
                continue
            self.medical_chat_seen.add(key)
            participants = ", ".join(record.get("participants", []))
            lines.append(
                f"## Step {record['step']} ({record['curr_time']})")
            if participants:
                lines.append(f"Participants: {participants}")
            lines.append("")
            for row in record.get("chat", []):
                if not row or len(row) < 2:
                    continue
                lines.append(f"- {row[0]}: {row[1]}")
            lines.append("")
            lines.append(f"<!-- chat-key: {key} -->")
            lines.append("")
        if lines:
            with open(self.medical_chat_path, "a") as outfile:
                outfile.write("\n".join(lines))
        ...
        while (True):
            ...
            if check_if_file_exists(curr_env_file):
                ...
                if env_retrieved:
                    ...
                    chat_records = self._extract_medical_chat_records(
                        movements)
                    self._append_medical_chat_records(chat_records)
        ...
        while True:
            ...
            try:
                if sim_command.lower() in ["f", "fin", "finish", "save and finish"]:
                    ...
                    curr_persona = self.personas[" ".join(
                        sim_command.split()[-2:])]
                    ...
                    self.personas[" ".join(
                        sim_command.split()[-2:])].s_mem.print_tree()
                    ...
                    cooordinate = [int(i.strip())
                                   for i in sim_command[16:].split(",")]
                    ...
                    rows = read_file_to_list(
                        curr_file, header=True, strip_trail=True)[1]
                    clean_whispers = []
                    for row in rows:
